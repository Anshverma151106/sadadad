...
if "hide_landing" not in st.session_state:
    st.session_state.hide_landing = False

if not st.session_state.hide_landing:
    st.markdown("""
    <div style="
        background: radial-gradient(1200px 600px at 20% 10%, rgba(0,255,156,0.15) 0%, rgba(0,255,156,0.02) 40%, transparent 70%),
                    radial-gradient(800px 800px at 90% 30%, rgba(0,150,255,0.15) 0%, rgba(0,150,255,0.02) 40%, transparent 70%);
        border: 1px solid rgba(255,255,255,0.06); 
        border-radius: 28px; 
        padding: 48px; 
        text-align: center;
        margin-bottom: 32px;">
        <div style="font-size: 12px; letter-spacing: 0.25em; text-transform: uppercase; color: rgba(255,255,255,0.6); font-weight: 800;">
            Enterprise Document Forensics
        </div>
        <h1 style="margin: 8px 0 12px 0; font-size: 48px; line-height: 1.1; color: #ededed;">
            Detect Fraud. Prove Authenticity. <span style="color:#00FF9C;">Instantly.</span>
        </h1>
        <p style="margin: 0 auto; max-width: 760px; color: rgba(255,255,255,0.7); font-size: 16px;">
            AI-powered integrity checks for signatures, copy-move edits, and deep forgery signals. 
            Built for compliance, banking, and eKYC at scale.
        </p>
        <div style="display:flex; gap: 12px; justify-content: center; margin-top: 24px;">
            <span style="background:#00FF9C; color:#000; font-weight:900; padding:10px 16px; border-radius:10px; font-size:12px; text-transform:uppercase; letter-spacing:0.12em;">Real‑time</span>
            <span style="background:rgba(255,255,255,0.06); color:#9ca3af; font-weight:800; padding:10px 16px; border-radius:10px; font-size:12px; text-transform:uppercase; letter-spacing:0.12em;">On‑device ELA</span>
            <span style="background:rgba(255,255,255,0.06); color:#9ca3af; font-weight:800; padding:10px 16px; border-radius:10px; font-size:12px; text-transform:uppercase; letter-spacing:0.12em;">Neural Heatmaps</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c1:
        with st.container(border=True):
            ...
            st.markdown("Writer‑independent Siamese embeddings with confidence scoring.")
            st.metric("Accuracy", "≈ 98%*", help="On CEDAR‑style datasets; varies with data.")
            if st.button("Start Verification", use_container_width=True, key="lp_sig"):
                st.session_state.hide_landing = True
                st.rerun()
    with c2:
        with st.container(border=True):
            st.markdown("### 🔍 Copy‑Move Detection")
            st.markdown("Keypoint matching and geometric verification with heatmaps.")
            st.metric("Mode", "ORB + RANSAC")
            if st.button("Scan for Copy‑Move", use_container_width=True, key="lp_cm"):
                st.session_state.hide_landing = True
                st.rerun()
    with c3:
        with st.container(border=True):
            st.markdown("### 🛡️ KAVACH AI Analysis")
            st.markdown("ELA‑preprocessed CNN classification with fraud heatmaps.")
            st.metric("Insight", "Neural + Signal")
            if st.button("Run AI Forensics", use_container_width=True, key="lp_ai"):
                st.session_state.hide_landing = True
                st.rerun()
    ...
    with col_a:
        with st.container(border=True):
            st.markdown("#### Why Kavach")
            st.markdown("- Unified pipeline for signatures, pixel edits, and noise anomalies")
            st.markdown("- Enterprise‑grade UI with clear verdicts and confidence")
            st.markdown("- Batch mode for operations teams and eKYC")
            if st.button("⚡ Start Free Scan", use_container_width=True, key="lp_start"):
                st.session_state.hide_landing = True
                st.rerun()
    with col_b:
        with st.container(border=True):
            st.markdown("#### Trusted For")
            st.markdown("- Banking & NBFC onboarding")
            st.markdown("- Insurance claims and audits")
            st.markdown("- Government e-doc verification")
...
def analyze_document(file_path: Path) -> dict:
    ...
    # 1. Run KAVACH AI Analysis if weights exist
...
with tab4:
    ...
    <div style="background: rgba(0, 255, 156, 0.05); border: 1px solid rgba(0, 255, 156, 0.1); border-radius: 16px; padding: 20px; margin-bottom: 32px;">
        <h3 style="margin: 0; color: #00FF9C;">KAVACH AI Engine</h3>
    ...
    if mode == "🖼️ Image Forgery":
        with st.container(border=True):
            if_file = st.file_uploader(
                "Upload document image for forensic scan", type=["png", "jpg", "jpeg", "tiff"], key="kavach_img"
            )
        ...
        if if_file:
            ...
            if run_btn:
                ...
                with st.spinner("Executing neural classification..."):
                    from src.analysis.ifake_tools import predict_ifake_forgery
                    res = predict_ifake_forgery(if_path, kavach_img_weights)
        ...
        with st.container(border=True):
            vf_file = st.file_uploader(
                "Upload video for frame analysis", type=["mp4", "avi", "mov"], key="kavach_vid"
            )
        if vf_file:
            st.markdown("<br>", unsafe_allow_html=True)
            st.video(vf_file)
            
            kavach_vid_weights = st.text_input(
                "Video Model Weights", value="weights/forgery_model_me.hdf5",
                help="Target .hdf5 weights for the ResNet50 video classifier.",
                key="kavach_vid_weights_path"
            )
            ...
            if st.button("⚡ Analyze Temporal Frames", use_container_width=True):
                ...
                with st.spinner("Processing frame-by-frame temporal analysis..."):
                    ...
                    res = detect_video_forgery(vf_path, kavach_vid_weights)
    ...
    st.divider()
    with st.expander("ℹ️  About KAVACH AI integration"):
        st.markdown("""
**KAVACH AI (Image Forgery Analysis and Knowledge Extraction)** provides additional forensic tools:

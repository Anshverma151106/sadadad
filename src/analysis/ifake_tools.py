"""
KAVACH AI forgery detection tools (IFAKE-inspired).
...
def init_ifake_classifier():
    """Initialize the KAVACH AI CNN classifier architecture (TensorFlow/Keras)."""
    if tf is None:
        return None
    try:
...
def predict_ifake_forgery(image_path: str | Path, model_path: str | Path | None = None) -> dict:
    ...
    if tf is None:
        return {
            "error": "TensorFlow is not installed on this system. KAVACH AI analysis is unavailable."
        }
    if model_path is None or not Path(model_path).exists():
        return {
            "error": (
                f"KAVACH AI model weights not found at `{model_path}`. "
    ...
    try:
        ...
        img_pil = Image.open(image_path).convert('RGB')
        ela_img = generate_ela(img_pil, quality=90, scale=1) # KAVACH AI uses scale=1 initially
        ...
        return {"error": f"Failed to load or run KAVACH AI model: {str(e)}"}
...
def detect_video_forgery(video_path: str | Path, model_path: str | Path | None = None) -> dict:
    ...
    if model_path is None or not Path(model_path).exists():
        return {
            "error": (
                f"KAVACH AI video model weights not found at `{model_path}`. "
    ...
    try:
        ...
        while cap.isOpened():
            ...
            # KAVACH AI preprocesses frames to 320x240
        ...
        return {"error": f"Failed to load or run KAVACH AI video model: {str(e)}"}
